...
DEX_NEEDLES = [
    ...
    "发文件给别的设备",            # 1.20：三张卡合成「发送」后留下的小节标题
    "点一下看文本内容",          # 1.12：收件箱文本条目的提示
    "创建设备组",                # 1.13：设备组卡片
    "加入设备组",
    "复制组 id",
    "解散设备组",
    "还没有加入任何设备组",      # 1.17：设备组卡片的状态行（首页文案全清后留下的最短提示）
    "刚刚活跃",                  # 1.18：设备组里的设备小卡（网页同款：图标+名字+活跃/收件箱+标签）
    "已有令牌？从剪贴板导入",     # 1.14：登记步骤并入建组/加入后，令牌认领改走这个次要按钮
    "已清除本机登记",             # 1.15：服务端说设备不存在时，本机缓存自动清掉
    "要只清除本机登记吗",
    "本机已在组里（你是管理员）",  # 1.14：建组成功提示（说明本机自动进组）
]
GONE_DEX_NEEDLES = [        # 界面上已经删掉的说明文字：不该再回到 dex 里（界面文案简化的回归护栏）
    "选 ShareLink，文件会先上传并给出分享码",
    "点下面的按钮就会自动把这台手机加进设备列表",
    "加入时用对方给的组 id",
    "点「管理」看成员与操作",
    "管理里可复制组 id",
    "只有同一个组里的设备之间才能互传",
