...
- **未登记时的那句「在相册 / 文件管理里点「分享」→ 选 ShareLink…」也在 v1.19 删掉了**，设备组卡里只剩
  「创建设备组 / 加入设备组」两个按钮（未登记时多一条「已有令牌？从剪贴板导入」）。
- **首页改版（v1.20）**：「上传有效期」「发文件给别的设备」「发文本」合成一张「发送」卡（内含三个小节）；
  「设备组」从最上面挪到「凭分享码下载」下面；「解散设备组」按钮标红（红色文字 + 淡红底）。
- **设备组摊平（v1.18）**：组里的设备改成网页端同款小卡（手绘矢量设备图标 + 名字 + 「刚刚活跃 · 收件箱 N 个文件」+
  「本设备 / 管理员 / 成员」标签 + 管理员的「移出」），组操作（复制组 id / 改组名 / 解散设备组 / 退出设备组）
  直接从卡片里点，**去掉了原来的「管理」二级弹窗**。组卡片渲染时会先拉一次 `/api/devices` 给成员补收件箱数。
- **首页文案精简（v1.16）**：设备组 / 上传有效期 / 发文件给别的设备 / 发文本 / 凭分享码下载
  五张卡片下面的说明文字都删掉了（按钮与标题已经能自解释）；「点下面的按钮就会自动把这台手机加进设备列表…」
  未登记时的那句「点下面的按钮就会自动把这台手机加进设备列表…」在 v1.17 也删掉了；
  设备组卡片的「共 N 个设备组」「N 台设备」保留，只去掉后面的说明（「点「管理」看成员与操作」「管理里可复制组 id…」）。
- **本机缓存过期自愈（v1.15）**：服务端不再认这台设备（管理员清理过 / 换了浏览器 / 重装）时，任何带令牌的请求都会拿到
  `404 device_not_found` / `403 bad_device_token`；App 立刻清掉本机存的设备与令牌、回到「创建设备组 / 加入设备组」，
  注销失败也给出「只清除本机登记」的出口。网页端同理（localStorage 的 `sharelink.device` 会被自动清掉）。
- **设备组（v1.13 起）**：首页「设备组」卡 → 创建设备组（创建者即管理员）/ 粘贴组 id 加入 / 点「管理」
