...
- **可装成手机应用（PWA）**：安卓 Chrome 直接装成独立应用，还能用系统分享面板把文件"分享 → ShareLink"；
  iOS Safari 添加到主屏幕；设备令牌可复制/粘贴导入（也能存成文件），换机或清缓存都能找回
...
- 设备标签页里的**设备令牌**：换手机或 iOS 清理站点存储后，用令牌找回设备。令牌直接以文本框显示、
  一键「复制令牌」；导入支持**粘贴 JSON**、**读取剪贴板**，也保留「选文件」。
  导入时会先读一次收件箱校验令牌，错的令牌不会被存下来。
  （手机上的主路径刻意不用文件：安卓 Chrome 的 blob 下载经常静默失败，不少国产 ROM 的文件选择器
  还会按类型过滤、根本不显示 `.json`；iOS Safari 对 `<a download>` 的 blob 也不支持。）
  同一个令牌也能粘进安卓 App 的「从剪贴板导入令牌」，两边格式互通。
...
网页版「设备」页的安装卡片里也直接放了「下载APK」按钮（地址就是上面这个链接）；
即便已经装成 PWA、安装引导收起来了，这个按钮也一直在（换手机或给别人的安卓机装时用得上）。
